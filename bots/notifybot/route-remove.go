...
func (nb *NotifyBot) routeRemove(m *pb.InMessage) {
	...
	if len(match) < 3 || match[2] == "" {
		_, err := nb.client.RespondToCommand(m, "I didn't understand this \"remove webhook\" message - is the webhook number or ID missing?")
		if err != nil {
			// Log errors only
	...
	if rows < 1 {
		_, err = nb.client.RespondToCommand(m, "I cannot find a webhook in this chat with that number or ID")
		if err != nil {
			// Log errors only
			nb.log.Printf("Error sending message to chat %d: %s\n", m.ChatId, err.Error())
			return
	...
	// All good
	_, err = nb.client.RespondToCommand(m, "Ok, I've removed the webhook from this chat")
	if err != nil {
		// Log errors only
		nb.log.Printf("Error sending message to chat %d: %s\n", m.ChatId, err.Error())
		return
